...
//=================================Ping VARs=============================================
    ...
    aiTrack.prototype.isLast = function() {
        return (this.questions.length - 1 === this.cur_index) ? 1 : 0;
    }

    aiTrack.prototype.totalMark = function() {
        return this.questions.length;
    }

    aiTrack.prototype.achievedMark = function() {
        return this.questions.reduce((achievedMark, cv) => {
            if (cv.correct) {
                achievedMark += 1;
            }
            return achievedMark;
    ...
    aiTrack.prototype.percentage = function() {
        const perc = Math.round((this.achievedMark() / this.totalMark()) * 100);
        return perc;
    }
    
    aiTrack.prototype.getQuestion = function(key) {
        return this.cur_item[key];
    }

    aiTrack.prototype.eachQuestion = function(runtime) {
  ...
  aiTrack.prototype.StartTimer = function ()
    ...
    aiTrack.prototype.CheckWork = function () {
        ...
        this.pingCount = this.pings.length;
        this.arrayQuestions = this.questions.map((oneQuestion, index) => {
            let questionString = `Question ${index + 1} `;
            for (let key of Object.keys(oneQuestion)) {
                questionString += `${key} = ${oneQuestion[key]}, `; 
            }
            questionString += "\n";
            return questionString;
        });

        parent.postMessage({ question: this.question, uidAnswers: newUidAnswers, uidClicks: newUidClicks, uidCorrectClicks: newUidCorrectClicks, uidClicksIntervals: newUidClicksIntervals, progress: this.progress, goal: this.goal, finished: this.finished, allCorrect: this.allCorrect, time: this.time, date: this.today, questions: this.questions}, '*');
        if (this.testing) {
